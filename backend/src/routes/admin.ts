...
import { createInvoice, getInvoiceByBookingId } from '../services/invoiceService';
  ...
  try {
    ...
    const fullBookingResult = await pool.query(
      `SELECT 
       ...
       b.payment_link, b.notes, b.coupon_code,
    ...
    // Generate invoice if status changed to 'confirmed' and invoice doesn't exist
    let invoicePath: string | null = null;
    if (currentBooking.status !== 'confirmed' && newStatus === 'confirmed') {
      try {
        // Check if invoice already exists
        const existingInvoice = await getInvoiceByBookingId(updatedBooking.id);
        
        if (!existingInvoice) {
          // Get booking extras for invoice
          ...
          const invoiceData: InvoiceData = {
            ...
            customerName: `${fullBooking.first_name} ${fullBooking.last_name}`,
            customerEmail: fullBooking.email,
            customerPhone: fullBooking.phone,
            customerAddress: fullBooking.address || undefined,
            customerCity: fullBooking.city || undefined,
            customerCountry: fullBooking.country || undefined,
            vehicleMake: fullBooking.make,
            vehicleModel: fullBooking.model,
            vehicleYear: fullBooking.year || undefined,
            pickupDate: new Date(fullBooking.pickup_date),
            dropoffDate: new Date(fullBooking.dropoff_date),
            pickupLocation: `${fullBooking.pickup_location_name}${fullBooking.pickup_location_city ? ', ' + fullBooking.pickup_location_city : ''}`,
            dropoffLocation: `${fullBooking.dropoff_location_name}${fullBooking.dropoff_location_city ? ', ' + fullBooking.dropoff_location_city : ''}`,
            basePrice: Number(fullBooking.base_price),
            extrasPrice: Number(fullBooking.extras_price || 0),
            discountAmount: Number(fullBooking.discount_amount || 0),
            taxRate: getTaxRate(),
            totalPrice: Number(fullBooking.total_price),
            extras: extras.length > 0 ? extras : undefined,
            couponCode: fullBooking.coupon_code || undefined,
          };

          // Create invoice
          const invoice = await createInvoice(updatedBooking.id, invoiceData);
          invoicePath = invoice.file_path;
          ...
          invoicePath = existingInvoice.file_path;
          console.log('[Admin] Invoice already exists for booking:', existingInvoice.invoice_number);
        }
      } catch (invoiceError: any) {
        // Don't block booking confirmation if invoice generation fails
        console.error('[Admin] Failed to generate invoice (non-blocking):', {
          error: invoiceError.message,
          booking_id: updatedBooking.id,
        });
      }
    } else {
      // Check if invoice exists for email attachment
      const existingInvoice = await getInvoiceByBookingId(updatedBooking.id);
      if (existingInvoice) {
        invoicePath = existingInvoice.file_path;
      }
